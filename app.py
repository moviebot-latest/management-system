...
def next_employee_id():
    def query():
        rows = User.query.with_entities(User.employee_id).all()
        max_n = 0
        for (eid,) in rows:
            try:
                n = int(eid[3:])
...
def _init_db():
    ...
    if admin_username and admin_password:
        ...
        if not existing:
            ok, _ = validate_password(admin_password)
            if ok:
                admin = User(
                    employee_id='EMP001',
                    ...
                    email=os.environ.get('ADMIN_EMAIL', admin_username + '@system.local'),
                ...
                db.session.commit()
                print('[INIT] Admin created: ' + admin_username, flush=True)
            else:
                print('[WARN] ADMIN_PASSWORD too weak, skipping admin seed.', flush=True)

# Runs at import time - works with Gunicorn + Render
with app.app_context():
    try:
        _init_db()
    except Exception as _e:
        print('[WARN] DB init: ' + str(_e), flush=True)
