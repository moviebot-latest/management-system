...
def migrate_book_schema():
    # Older versions of this project used book.book_code as the primary key.
    # The current ORM uses integer book.id plus the human-readable book_id.
    # Upgrade the existing PostgreSQL schema in-place so existing books/loans
    # are preserved. SQLite/local fresh installs need no migration.
    ...
    try:
        ...
        # Old schema: book_code -> current model name: book_id.
        ...
        # The block above is intentionally followed by a simpler, reliable FK
        # cleanup for this project's loan table.
                ...
                IF to_regclass('public.loan') IS NOT NULL THEN
                    FOR r IN
                        ...
                        WHERE conrelid='public.loan'::regclass
                          ...
                          AND contype='f'
                    LOOP
                        EXECUTE format('ALTER TABLE loan DROP CONSTRAINT %I', r.conname);
                    END LOOP;
                END IF;
            END $$;
        """))

        # Add the integer ORM primary-key column only when it is missing.
        if 'id' not in book_cols:
            db.session.execute(text(
                'ALTER TABLE book ADD COLUMN id INTEGER GENERATED BY DEFAULT AS IDENTITY'
            ))

        # If an old loan table stores the human-readable book code as text,
        # convert those references to the new integer book.id values.
        ...
        # Make book.id the real database primary key, matching the ORM.
        db.session.execute(text("""
            DO $$
            DECLARE r record;
            BEGIN
                ...
                IF NOT EXISTS (
                    ...
                    WHERE conrelid='public.book'::regclass
                      ...
                      AND conkey = ARRAY[(SELECT attnum FROM pg_attribute
                                         WHERE attrelid='public.book'::regclass
                                           AND attname='id')]
                ) THEN
        ...
        # Keep the public book code unique after the primary-key swap.
        db.session.execute(text("""
            DO $$
            BEGIN
                IF NOT EXISTS (
        ...
        # Restore the current application's loan -> book relationship.
        db.session.execute(text("""
            DO $$
            BEGIN
